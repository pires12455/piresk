...
ʙᴇᴍ ᴠɪɴᴅᴏ ᴀ ᴇsᴛᴇ ɢʀᴜᴘᴏ, ᴍᴇɴᴏs ɴᴏᴛᴀ ғᴀᴋᴇ ᴇ ᴢᴇ ᴛʀᴀᴠɪɴʜᴀ 
 ...
 *${mdata.subject}*\n\n ❤️`
				...
				teks = `𝙁𝙡𝙬, *meu caro garfanhoto*@${num.split('@')[0]} 😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["556996103915@s.whatsapp.net","556996103915@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"558981210976@s.whatsapp.net","556996103915@s.whatsapp.net"]//ubah nomor lo
...
// recursos anime, random @Causs domina vida
          ...
          reply('Espere..')
				...
				case 'hentai':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Cara bate pra 2d kkkkkkkk'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
			...
			reply(mess.wait)
				...
				case 'bc':
					...
					if (args.length < 1) return reply('ta.')
					anu = await client.chats.all()
